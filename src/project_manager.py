...
class ProjectManager:
    ...
    def save_chapter_files(self, project_folder: str, chapter_number: int,
                           chapter_text: str, story_context: str, character_relations: str):
        """Save chapter body + overwrite context files."""
        chapter_path = os.path.join(project_folder, "chapters", f"{chapter_number}화.txt")
        with open(chapter_path, "w", encoding="utf-8") as f:
            f.write(chapter_text)
        with open(os.path.join(project_folder, "context", "story_context.md"), "w", encoding="utf-8") as f:
            f.write(story_context)
        with open(os.path.join(project_folder, "context", "character_relations.md"), "w", encoding="utf-8") as f:
            f.write(character_relations)
        logger.info("챕터 %d화 저장 완료: %s", chapter_number, project_folder)
